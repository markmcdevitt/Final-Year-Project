	...
	<c:forEach var="user" items="${userList}">
		...
		<c:if test="${user.weeklyPlan.size()>0}" var='0'>
			...
			<c:forEach var="weeklyplan" items="${user.weeklyPlan}">
				...
				<td><form
						action="${pageContext.request.contextPath}/deleteUsersDay/${weeklyplan.id}">
				...
				<c:forEach var="recipe" items="${weeklyplan.recipe}">
					<td><a
						...
						<form
							action="${pageContext.request.contextPath}/deleteRecipeFromPlan/${weeklyplan.id}/${recipe.titleParse}">
							<button type="submit" id="button"
								class="btn btn-secondary btn-sm" value="">&#10006</button>
						</form></td>
				</c:forEach>
			</c:forEach>
		</c:if>
...
<script>
	$.getScript('http://arshaw.com/js/fullcalendar-1.6.4/fullcalendar/fullcalendar.min.js',
					function() {
						var weeklyPlan ='<c:out value="${user.weeklyPlan}"/>';
						for(var i =0;i<weeklyPlan.length;i++){
							var d = '<c:out value="${weeklyplan.date[1]}"/>';
							var date = new Date(d);
							alert('date: ' + date);
						}
						var date = new Date();
